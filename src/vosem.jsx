...
const Lesson10 = () => {
    ...
    let deteiC = async () => {
        ...
        let detData = await axios({
            url: `https://api.themoviedb.org/3/tv/${param.id}?api_key=${apikey}&language=ru-RU&page=1`
        })
    ...
    let videiC = async () => {
        let videtData = await axios({
            url: `https://api.themoviedb.org/3/tv/${param.id}/videos?api_key=${apikey}&language=en-US`
        })
    ...
    let similarC = async () => {
        let similarData = await axios({
            url: `
            https://api.themoviedb.org/3/tv/${param.id}/similar?api_key=${apikey}&language=ru-RU&page=1`
        })
        if (similarData != null) {
    ...
    let recomen = async () => {
        let recomenData = await axios({
            url: `https://api.themoviedb.org/3/tv/${param.id}/recommendations?api_key=${apikey}&language=ru-RU&page=1`
        })
        if (recomenData != null) {
            ...
            <div>
                ...
                <div className="col-lg-11 mx-auto">
                    <div className="row mt-5 ">
                                                    ...
                                                    {detail.seasons
                                                        .map(i => <>
                                                            <div className="col-2">
                                                                ...
                                                                <img width={'50px'} className="rounded-5" height={'50px'} src={`https://www.themoviedb.org/t/p/w600_and_h900_bestv2${i.poster_path}`} alt="" />

                                                                <span> {i.name}</span>
                                                                <span>{i.number}</span>
                                                                <span>{i.overview} </span> <br /> <br /></div>
                                                        </>)}</div></div>
                                        </p>
                        ...
                        <div className="col-lg-2 exo  text-center ">
                            ...
                            {similar != null ?
                                similar.map(i =>
                                    <>
                                        ...
                                        <a href={"/detail/" + i.id}><h5>{i.name}</h5></a>
                                        <p>{i.release_date}</p>
                                    </>
                                    ) : <> loading </>}
                        </div>
                    ...
                    <div className="col-lg-12  exos d-flex text-center">
                        {recomendate != null ?
                            recomendate.map(i =>
                                <>
